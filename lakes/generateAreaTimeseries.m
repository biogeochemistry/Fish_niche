...
function generateAreaTimeseries(lakelistfile,m1,m2,exA,y1A,exB,y1B,outputdir)
...
timeseries_records{1,2} = 'Average water area above min T gradient and over secchi gradient';
...
if i >= 1
	exportAreaTimeseries(sprintf('%s/fish_niche_Area_export_%s.csv',outputdir,d4),timeseries_records,startdate)
	end
end
