function export_isimip_params_lake(lake_params,parfileplace, parfile,variable_calibrated)
    ...
    if exist(basicfile, 'file') == 2
        ...
        if(variable_calibrated == "temperature")
            while ischar(tline)
                i = i+1;
                if( i == 6 || i== 7 || i == 10 || i== 11 || i== 18 || i == 19 || i == 26|| i == 27)
                    j = i-2;
                    tline = fgets(fid);
                    sc = split(tline);
                    ...
                    t3=num2str(sc{5});
                    t4=sc{6};
                    tline = sprintf('            %s\t%s\t%s\t%s\t%s',sc{2},num2str(cell{1,1}),num2str(sc{4}),num2str(sc{5}),sc{6});
                else
                    j = i-2;
                    tline = fgets(fid);
                    if j >= 61
