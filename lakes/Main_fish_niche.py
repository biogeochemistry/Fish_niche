...
lakes = pd.read_csv("2017SwedenList.csv",
...
def run_calibrationsoxy(lake_number, lakesList12="2017SwedenList_only_validation_12lakes.csv"):
    ...
    if test == False:
        ...
        if not os.path.exists("{}/2020input".format(lake.calibration_path)):
            ...
            icedays = list(lakes["icemean"])[lake_number]
            lake = CalibrationInfo(lakes_list[lake_number], list(lakes["lake_id"])[lake_number],
                                   list(lakes["subid"])[lake_number], list(lakes["ebhex"])[lake_number],
                                   list(lakes["area"])[lake_number], list(lakes["depth"])[lake_number],
                                   list(lakes["Mean"])[lake_number],
            ...
            cmd = "matlab -wait -r -nosplash -nodesktop MyLake_optimizer_final(%d,%d,%d,'%s','%s','%s','%s',%f,%f,%f,'%s');quit" % \
                  (lake.start_year, lake.end_year, lake.spin_up, os.path.join(lake.calibration_path, "2020par"),
                   ...
                   lake.calibration_path, lake.latitude, lake.longitude, icedays, "oxygen")
...
def calibration_iteration(lakesList12="2017SwedenList_only_validation_12lakes.csv"):
    ...
    for lake_number in range(0, len(lakes_list)):
        if lake_number in [13]:
            print('here')
            run_calibrations(lake_number)
    #     if lake_number in [7,8,9]:
    #         run_calibrationsoxy(lake_number)
    # for lake_number in range(0,len(lakes_list)):
    #     if not lake_number in [13]:
    #         print('here')
    #         run_calibrations(lake_number)
    for lake_number in range(0, len(lakes_list)):
        if lake_number in [0, 1]:
            run_calibrationsoxy(lake_number)
...
def run_calibrations(lake_number, lakesList12="2017SwedenList_only_validation_12lakes.csv", old=False):
    """
        Intermediary function to call Nelder-mead optimization function for a single lake.
...
def summary_characteristics_lake_parallel(modelid=2, scenarioid=2, lakes_listcsv="2017SwedenList.csv",
    ...
    Parallel(n_jobs=num_cores, verbose=10)(
        delayed(summary_characteristics_lake1)(lake_number, lakes_listcsv, modelid, scenarioid, calibration, old) for
...
def summary_characteristics_lake1(lake_number, lakes_listcsv="2017SwedenList.csv", modelid=2, scenarioid=2,
    ...
    if str(list(lakes["Mean"])[lake_number]) != "nan":
        try:
            ...
            #         final_equation_parameters(list(lakes["longitude"])[lake_number], list(lakes["latitude"])[lake_number],
            ...
            #                                   list(lakes["volume"])[lake_number], list(lakes["sedimentArea"])[lake_number])
            #     lakeinfo = LakeInfo(lakes_list[lake_number], list(lakes["lake_id"])[lake_number],
            #                         list(lakes["subid"])[lake_number], list(lakes["ebhex"])[lake_number],
            #                         list(lakes["area"])[lake_number], list(lakes["depth"])[lake_number],list(lakes["Mean"])[lake_number],
            #                         list(lakes["longitude"])[lake_number], list(lakes["latitude"])[lake_number],
            #                         list(lakes["volume"])[lake_number], list(lakes["Turnover"])[lake_number],
...
def summary_characteristics_lake(modelid=2, scenarioid=2, lakes_listcsv="2017SwedenList.csv", calibration=False,
                ...
                if os.path.exists(os.path.join(outdir, "strat.csv")) and os.path.exists(
                    ...
                    if list(lakes["lake_id"])[lake_number] in [32276, 310, 14939, 30704, 31895, 6950, 99045, 33590,
                        ...
                        if calibration:  # try:
                            ...
                            # print(cmd)
                            # os.system(cmd)
                            # lakeinfo.comparison_obs_sims_new(calibration=True,thermocline=thermocline)
                        # except:
                        #     print("issues")
    ...
    if withfig:
        Graphics(outputfolder=output_path).density_plot(summary, calibration, old)
...
def summary_characteristics_lake_summary(models, scenarios, lakes_listcsv="2017SwedenList.csv", calibration=False,
        ...
        for scenarioid in scenarios:
            for lake_number in range(0, len(lakes_list)):
                if 1 == 1:  # try:
                    ...
                    if os.path.exists(os.path.join(lakeinfo.outdir, '20210531REDOCOMPLETE')):
                        summary.loc[i, "run_completed"] = 1
                        # lakeinfo.outputfile(calibration=True)
                    else:
                        summary.loc[i, "run_completed"] = 0
    ...
    import time
    timestr = time.strftime("%Y%m%d-%H%M%S")
    summary.to_csv("summary_info_lake_lakes_summary_%s.csv" % (timestr), index=False)
...
def comparison_plot(lakes_listcsv="2017SwedenList.csv", modelid=2, scenarioid=2, outputfolder=r'F:\output'):
    # lakes_list = list(lakes_data.get("name").values())
    lakes = pd.read_csv(lakes_listcsv, encoding='ISO-8859-1')  # _only_validation_12lakes.csv", encoding='ISO-8859-1')
    lakes_data = lakes.set_index("lake_id").to_dict()
    lakes_list = list(lakes_data.get("name").keys())
    ...
    label_method = ["Old_Calibration", "New_Calibration", "Estimation"]
    # lakes_list = list(lakes_data.get("name").values())
    position = -1
    lakeslist = []
    for lake_number in range(0, len(lakes_list)):
                ...
                if calibration or (not calibration and not old):
                    if list(lakes["lake_id"])[lake_number] in [6950, 32276, 310, 14939, 30704, 31895, 99045, 33590,
                        ...
                        if 1 == 1:  # os.path.exists(os.path.join(lakeinfo.calibration_path,
                            # "Calibration_Complete.csv")):  # or outputfolder == r"C:\Users\macot620\Documents\GitHub\Fish_niche\output_calibration":
                            # outdir = lakeinfo.outdir
...
def FishNiche_TO_graph(lakes_listcsv="2017SwedenList.csv", calibration=False, modelid=2, scenarioid=2,
                       outputfolder=r'F:\output', old=False):
    # lakes_list = list(lakes_data.get("name").values())
    lakes = pd.read_csv(lakes_listcsv, encoding='ISO-8859-1')  # _only_validation_12lakes.csv", encoding='ISO-8859-1')
    lakes_data = lakes.set_index("lake_id").to_dict()
    lakes_list = list(lakes_data.get("name").keys())
                ...
                if calibration:
                    ...
                    swa_b1, swa_b0, c_shelter, i_sct, i_scv, i_sco, i_scdoc, k_sod, k_bod, kzn0, albice, albsnow = \
                        final_equation_parameters(list(lakes["longitude"])[lake_number],
                                                  ...
                                                  list(lakes["volume"])[lake_number],
                                                  list(lakes["sedimentArea"])[lake_number])
                    if old:  # outputfolder == r"C:\Users\macot620\Documents\GitHub\Fish_niche\output_calibration":
                        lakeinfo = LakeInfo(lakes_list[lake_number], list(lakes["lake_id"])[lake_number],
                                            list(lakes["subid"])[lake_number], list(lakes["ebhex"])[lake_number],
                                            list(lakes["area"])[lake_number], list(lakes["depth"])[lake_number],
                                            list(lakes["Mean"])[lake_number],
                                            ...
                                            list(lakes["volume"])[lake_number], list(lakes["Turnover"])[lake_number],
                                            scenarioid=scenarioid, modelid=modelid, calibration=calibration,
                                            outputfolder=r'F:\output', old=old)
                ...
                if os.path.exists(os.path.join(lakeinfo.calibration_path,
                    ...
                    if os.path.exists("{}/Tztcompare.csv".format(outdir)):
                        if 1 == 1:  # try:
                            ...
                            results = Graphics(output_path).graphiqueTO(x=lake_temp_data, y=lake_temp_model, z=lake_temp_depth,
                                                  symbol=lake_temp_lake,
                                                  r_value=r_value, slope=slope, intercept=intercept,
                    ...
                    if os.path.exists("{}/O2ztcompare.csv".format(outdir)):
                        if 1 == 1:  # try:
                            ...
                            results = Graphics(output_path).graphiqueTO(x=lake_oxy_data, y=lake_oxy_model, z=lake_oxy_depth,
    ...
    results = Graphics(output_path).graphiqueTO(x=all_temp_data, y=all_temp_model, z=all_temp_depth, symbol=symboltemp, r_value=r_value,
                          slope=slope, intercept=intercept, variable="Temperature (°C)", calibration=calibration,
                          old=old, outputfolder=outputfolder1)
    ...
    results = Graphics(output_path).graphiqueTO(x=all_oxygen_data, y=all_oxygen_model, z=all_oxygen_depth, symbol=symboloxy, r_value=r_value,
                          slope=slope, intercept=intercept, variable="Oxygen (mg/L)", calibration=calibration, old=old,
...
if __name__ == "__main__":
    ...
    # data_descriptif()
    # run_calibrations(11)
    # calibration_iterationoxy()
    # calibration_iteration5()
    # calibration_iterationoxy5()
    # calibration_iteration()
    # calibration_iterationoxy()
    # violin_plot()
    # violin_parallel()
    ...
    # summary_characteristics_lake(2,2,lakes_list1)
    ...
    # summary_characteristics_lake_summary(calibration=True)
    # FishNiche_mean_secchi_graph(calibration=True)
    # FishNiche_mean_secchi_graph(calibration=False)
    # FishNiche_TO_graph(calibration=False)
    # exploratory()
    ...
    # violin_plot("85", "2017SwedenList.csv", r"F:\output")
    # violin_plot45("2017SwedenList.csv", r"F:\output")
    ...
    # summary_characteristics_lake_summary([2],[2], lakes_listcsv="2017SwedenList_only_validation_12lakes.csv", calibration=True,
    #                                      old=True)
    # summary_characteristics_lake_summary([2],[2], lakes_listcsv="2017SwedenList_only_validation_12lakes.csv", calibration=False,
    #                                      old=False)
