...
lakelistfile  = '2017SwedenList_oxygen_validation.csv';
outputdir = '../output';
...
for ii=1:nlakes
    ...
    lakes(ii).outputfile = strcat(outputdir2, d4, '/O2zt.csv');
...
% For temperature
varyindexes = [23, 47, 48, 49, 50, 51, 52];
initialparam = [1, 0.01, 100, 1, 1, 1, 1];

% Best so far:
% 0.420 0.005 425.074 1.058 1.054 1.084 1.068
...
function error = FishNiche_single_run(main_scriptfile, lakelistfile, templatefile, lakedata, varyindexes, varyparam)
    ...
    %period. Could be done manually by just using a differen lake file.
