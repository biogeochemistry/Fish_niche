...
timeseries_records{1,8} = 'Volume with PPFD > 100';
	...
	try
        ...
        for time = 1:tlen
			...
			for depth = 1:zlen
				...
				%end
				if O2(time, depth) > 3000 && T(time, depth) < 15 && PARzt(time,depth) > 100
...
if i >= 1
	exportVolumeTimeseries(sprintf('%s/fish_niche_export100_%s.csv',outputdir,d4),timeseries_records,startdate)
	end
end
