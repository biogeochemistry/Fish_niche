...
timeseries_records{1,8} = 'Volume with Iz > 8';
...
for lakenum = 1:nlakes
	...
	try
		...
		volume_Iz_above_8 = zeros(tlen, 1);
        ...
        for time = 1:tlen
			...
			for depth = 1:zlen
				Iz_calculated(time,depth) = (Qst(time)*exp(-meanlambda(time)*depth))*(1-Ice(time));%Iz
				...
				%end
				if O2(time, depth) > 3000 && T(time, depth) < 15 && Iz_calculated(time,depth) >= 8
		...
		timeseries_records{1+lakenum, 8} = volume_Iz_above_8;
