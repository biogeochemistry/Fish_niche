...
cordexfolder = 'E:\cordex' #5-24-2018 MC # needs to be changed depending where the meteorological files are.
...
def mylakeinput(pA, pB, datesA, datesB, eh, subid, inflowfile, outpath,eh1,subid1):
    """
    ...
    if pA['clt'].find('MOHC-HadGEM2-ES') != -1 and pA['clt'].find('r1i1p1_SMHI-RCA4_v1_day')!= -1 : # 5-24-2018 MC
        dfmissingdata = pd.concat([take5_missingdata(pA, datesA, eh,eh1), take5_missingdata(pB, datesB, eh,eh1)]) #5-24-2017 MC
        if datesB[-1] == pd.datetime(2099,11,30):#2018-08-01 MC
            dfmissingdata= dfmissingdata[:-31]
        df = dfmissingdata.interpolate() # 5-24-2018 MC
    else:
        df = pd.concat([take5(pA, datesA, eh,eh1), take5(pB, datesB, eh,eh1)])

    ndays = len(datesA) + len(datesB)
    df.index = np.arange(ndays)
    dflow = inflow5(inflowfile,  datesA + datesB , subid1)
...
def runlake(modelid, scenarioid, eh, subid, depth, area, longitude, latitude,k_BOD=0.01,swa_b1=1,k_SOD=100,I_scDOC=1,report=None):
    ...
    if os.path.exists ( os.path.join ( outdir, '2017REDOCOMPLETE' ) ):
        ...
        #test with always same input data for dis lake
        ebh='0xbc3bb8'
        ebh.strip('"')
        eh1 = ebh[2:] if ebh[:2] == '0x' else ebh
        while len ( eh1 ) < 6:
            eh1 = '0' + eh1
        subid1=int('9824')
        ...
        mylakeinput ( pA, pB, datesA, datesB, eh, subid, inflowfilename, inputp,eh1,subid1)
