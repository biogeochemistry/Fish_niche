"""
FishNiche_output_analysis_scripts.py
last modification: 2018-08-30 MC

functions used to produce different figures and files
...
datafolder = r'G:\output-08-10-2018'
...
def graphique(x, y, xerr, yerr, coeff, r_value, variable_test, slope, intercept, variable_analyzed):
    ...
    plt.plot([line_start, lineEnd], [line_start, lineEnd], 'k-', color='b', label="1x+0")
    ...
    ax.plot(x_pred, y_pred, '-', color='k', linewidth=2,
...
def area_at_depth(surface_area,depth_pos,max_depth):
    """
    calcul the volume at a certain depth (area*depth=1).
    using cone, calculate the proportional area at the depth_pos.
        ...
        if int(lake_id) in lakeswehavedatafor:
            ...
            if crashed :
                ...
                if not anydata_o:
                    ...
                    RMSE_O = round( sqrt( sklearn.metrics.mean_squared_error( O2_data_samples, O2_model_samples ) ), 2 )
...
def FishNiche_csv_results_revise(scenarioid,modelid,lakelistfile,calivari,k_BOD):
    ...
    #lakeswehavedatafor = [698]
    lakeswehavedatafor = [14939,67035,16765,698,33494,33590,99045,30704, 19167, 31895, 310, 32276, 99045, 99516, 6950,33590,33494]
    #lakeswehavedatafor = [698, 67035, 31895, 310, 32276]
    ...
    for lakenum in np.arange(1, nlakes):
        ...
        if int(lake_id) in lakeswehavedatafor:
            ...
            if anydata_o:
                ...
                points=plt.scatter([i/1000 for i in O2_data_samples],[i/1000 for i in O2_model_samples],label=label_o,alpha=alpha,s=size,linewidths=lw,c=color,edgecolors='k',cmap='Reds_r',marker=mark)
                ...
                ax = plt.gca ()
                plt.subplot(rows, cols,lakenum)
                points=plt.scatter([i/10**3 for i in O2_data_samples],[j/10**3 for j in O2_model_samples],c=O2_depths,edgecolors='k', label=label_o, s=50,cmap='bone_r')
...
def FishNiche_generate_timeseries(filename,scenarioid,modelid):
    ...
    for lakenum in np.arange(1,nlakes):
        ...
        tlen,zlen = T.shape
        average_o2_above, average_T_below, average_o2_below, average_T_above = [0]*tlen,[0]*tlen,[0]*tlen,[0]*tlen
        volume_o2_above,volume_T_below,volume_Attn_above,volume_all_parameters = [0]*tlen,[0]*tlen,[0]*tlen,[0]*tlen
        ...
        for time in np.arange(0,tlen):
            ...
            for depth in np.arange(0,zlen):
                ...
                if o2.loc[time,depth] > 3000 and T.loc[time,depth] < 15 and Attn.loc[time,depth] >= 0.01 * surface_attn:
                    volume_all_parameters[time]= volume_all_parameters[time] + area_at_depth(surface_area,depth,max_depth)
...
def FishNiche_plot_timeseries(lakelistfile,scenarioid,modelid,calivari,outputfile):
    ...
    for lakenum in np.arange ( 1, nlakes ):
        ...
        time = pd.to_datetime (time, format='%d.%m.%Y')  #MC 2018-08-15,format='%Y-%m-%d') with . = format produce by matlab, - = python
...
def FishNiche_cathegories(lakelistfile):
    ...
    p=plt.hist ( final["Volume"], bins=np.logspace ( np.log10 (247419.07882405), np.log10 (1.75631E+11), 29),color="#3F5D7D",edgecolor = 'k' )
        ...
        for scenario in ['historical','rcp45','rcp85'] :
            ...
            for model in [1,2, 3, 4,5,6]:
                ...
                #ax1.plot_date(tt, meanbyday['%_T']-margin_of_error['%_T'],'k--',lw=2, ms=3)
                    ...
                    ax1.plot_date ( tt, meanbyday['%0_PAR2'],  '-',color=plt.cm.binary((model/10)+0.3), lw=2, ms=3 )
                ...
                #ax1.plot_date ( tt, meanbyday['%_PAR'] - margin_of_error['%_PAR'], 'g--', lw=2, ms=3 )
                    ...
                    plt.yticks ( [0,25, 50,75, +100] )
                    ax4.plot_date ( tt, meanbyday['%0_O2'],  '-',color=plt.cm.binary((model/10)+0.3), lw=2, ms=3 )
            ...
            for i in [1,2,3,4,5,6]:
                interet.append ( mlines.Line2D ( [], [], color=plt.cm.binary ( (i / 10) + 0.3 ), markersize=3,label=modelname[i - 1] ) )
            ...
            first_legend = plt.legend ( loc='upper center', bbox_to_anchor=(0.4,-0.2), fancybox=True, shadow=True,ncol=1, handles=ert )
            plt.gca ().add_artist ( first_legend )

            plt.legend (loc='upper center', bbox_to_anchor=(0.5, -0.2),fancybox=True, shadow=True, ncol=1, handles=interet )
            plt.show()
            #fig1.savefig ( path.join ( datafolder, "Figure_synthese_A2_group_%s_scenario_%s_mean.png" % (group, scenario) ) )
            fig1.savefig ( path.join ( datafolder, "Figure_synthese_A2_group_%s_scenario_%s_mean.svg" % (group, scenario) ) )
            print('completed')
...
def FishNiche_plot_volume_param(param,lakelistfile,listscenarios,listmodels,calivari,datafolder):
    ...
    for group in [3,1]:
        ...
        drange = [list ( range ( 1971, 1981) ),list ( range ( 2001,2011) ),list ( range ( 2031, 2041) ),list ( range ( 2061, 2071)),list ( range ( 2091, 2101) )]
        ...
        for k in [1,2,3,4,5,6]:
            interethis.append ( mlines.Line2D ( [], [], color=plt.cm.Blues ( (k / 10) + 0.3 ), markersize=3,label=modelname[k - 1] ) )
        for i in [1,2,3,4,5,6]:
            interet45.append(mlines.Line2D ( [], [], color=plt.cm.binary((i/10)+0.4), markersize=3,label=modelname[i - 1] ))
        for j in [1,2,3,4,5,6]:
            interet85.append(mlines.Line2D ( [], [], color=plt.cm.Wistia((j/10)+0.067*j), markersize=3,label=modelname[j - 1]))
...
def generate_timeseries_his_by_model(listmodels,listscenarios,lakelistfile,datafolder):
    ...
    for model in listmodels:
        for scenario in listscenarios:
            ...
            if path.exists ( path.join ( datafolder, 'fish_niche_export_His_EUR-11_%s_%s-%s_%s_%s0101-%s1231.csv' % (
                ...
                timeseries = pd.DataFrame(columns=['Date','Model','Scenario','Lake_group','rho_snow','IceIndicator','Total Volume'])
    ...
    complete_data.loc[complete_data['Lake_group']==1].to_csv( path.join( datafolder, 'His_data_12.csv' ), index=False )
    print('1_save')
    complete_data.loc[complete_data['Lake_group']==2].to_csv( path.join( datafolder, 'His_data_22.csv' ), index=False )
    print('2_save')
    complete_data.loc[complete_data['Lake_group']==3].to_csv( path.join( datafolder, 'His_data_32.csv' ), index=False )
    print('end')
...
def generate_timeseries_by_model(listmodels,listscenarios,lakelistfile,datafolder):
    ...
    for model in listmodels:
        for scenario in listscenarios:
            ...
            #print(tzt_dir)
            if not path.exists ( path.join ( datafolder, 'fish_niche_export_EUR-11_%s_%s-%s_%s_%s0101-%s1231.csv' % (
            m1, exA, exB, m2, y1A, y2B) ) ):
                ...
                #        (lakelistfile, m1, m2, exA, y1A, exB, y1B, datafolder)
                # print ( cmd )
                # os.system ( cmd )
                print('nan')
            else:
                ...
                timeseries_select= pd.DataFrame(columns=['Date','Model','Scenario','Lake_group','%_T','%_O2','%_PAR','Total Volume'])
                ...
                if i==0:
                    ...
                    print('added')
    complete_data.loc[complete_data['Lake_group']==1].to_csv( path.join( datafolder, 'complete_data_1.csv' ), index=False )
    print('1_save')
    complete_data.loc[complete_data['Lake_group']==2].to_csv( path.join( datafolder, 'complete_data_2.csv' ), index=False )
    print('2_save')
    complete_data.loc[complete_data['Lake_group']==3].to_csv( path.join( datafolder, 'complete_data_3.csv' ), index=False )
    print('end')
...
def FishNiche_graph_temp_time(scenarioid,modelid,lakelistfile,k_BOD,i):
    ...
    for lakenum in np.arange(1, nlakes):
        ...
        if int(lake_id) in lakeswehavedatafor:
            ...
            outdir = path.join( outputfolder, d1, d2, d3, 'EUR-11_%s_%s-%s_%s_%s0101-%s1231\\Tzt.csv' % (m1, exA, exB, m2, y1A, y2B) )
            print(outdir)
            ...
            if int ( lake_id ) == 698:
                plt.subplot(211)
                t_0_2.plot(style=['k-'],markersize=3,label="1-2 m"); plt.legend(loc='best')
                t_25.plot(style=['r-'],markersize=3,label="25 m"); plt.legend(loc='best')
                td_02.plot(style=['go'],markersize=5,label="1-2 m data"); plt.legend(loc='best')
                td_25.plot ( style=['C4o'], markersize=5, label="25 m data" );plt.legend(loc='best')
            ...
            if lake_id == '67035':
                plt.subplot(212)
                t_05.plot(style=['k-'],markersize=3,label="0.5m"); plt.legend(loc='best')
                t_20.plot ( style=['r-'], markersize=3,label="20 m" );plt.legend ( loc='best' )
                t_60.plot (  style=['b-'], markersize=3,label = "60 m");plt.legend ( loc='best' )
                td_01.plot ( style=['go'], markersize=5, label="0.5 m data" );plt.legend ( loc='best' )
                td_20.plot ( style=['C4o'], markersize=5, label=">20 m data" );plt.legend ( loc='best' )
                td_60.plot ( style=['C1o'], markersize=5, label=">60 m data" );plt.legend ( loc='best' )
                plt.legend(loc = 'upper center', bbox_to_anchor = (0.5, -0.05),fancybox = True,ncol = 5)
...
def FishNiche_graph_oxy_time(scenarioid,modelid,lakelistfile,swa_I_scDOC,k_BOD):
    ...
    for lakenum in np.arange(1, nlakes):
        ...
        if int(lake_id) in lakeswehavedatafor:
            ...
            outdir = path.join( outputfolder, d1, d2, d3, 'EUR-11_%s_%s-%s_%s_%s0101-%s1231\\O2zt.csv' % (m1, exA, exB, m2, y1A, y2B) )
            ...
            if test1.empty != True:
                ...
                if td_02.empty != True and td_03.empty != True:
                    if int(lake_id) == 698:
                        ...
                        winter_months = tt[((tt.year==2001)&(tt.month==1)&(tt.day==1))| ((tt.month == 12)&(tt.day == 21))
                                           | ((tt.month == 3)&(tt.day == 20))|((tt.year==2010)&(tt.month==12)&(tt.day==31))]
                        summer_months = tt[((tt.month ==6) & (tt.day == 21))| ((tt.month == 9) & (tt.day == 22))]
                        # Loop through groups and add axvspan between winter month
                        for i in np.arange(0,len(winter_months),2):
                ...
                if not anydata_secchi:
                    ...
                    for i in np.arange(0,len(secchi_model_samples[0])):
                        RMSE_secchi.append(round ( sqrt ( sklearn.metrics.mean_squared_error ( secchi_data_samples,
                                                                                               [item[i] for item in secchi_model_samples] )
                                                          ), 2 ))
                        corr_secchi.append( scipy.stats.pearsonr ( secchi_data_samples, [item[i] for item in secchi_model_samples] ) )
...
def FishNiche_mean_secchi_graph(scenarioid,modelid,lakelistfile,calivari,variable_test):
    ...
    for i in np.arange (0,len(coefficients)):
        ...
        #plt.ylim ( 0, 16 )
        #plt.xlim(0,16)
        results = graphique ( all_mean_secchi_data, mean_model_data, all_std_secchi_data, std_model_data, coefficients[j], r_value,
                    variable_test, slope, intercept, 'SECCHI' )
...
if __name__ == '__main__':
    ...
    #FishNiche_validate_results(2, 4, r'C:\Users\Marianne\Documents\Fish_niche1\MDN_FishNiche_2017\lakes\2017SwedenList_only_validation_lakes.csv', 'Final', 0.0001)
    # i = 0.5
    ...
    FishNiche_plot_volume(r'C:\Users\Marianne\Documents\Fish_niche\MDN_FishNiche_2017\lakes\
         2017SwedenList.csv', [1, 2, 3, 4, 5, 6], [1, 2, 3, 4, 5, 6, 7, 8], 1, datafolder)
    #
    # generate_timeseries_his_by_model([1, 2, 3, 4, 5, 6], [1, 2, 3, 4, 5, 6, 7, 8], r'C:\Users\Marianne\Documents\
    #     Fish_niche\MDN_FishNiche_2017\lakes\2017SwedenList.csv', datafolder)