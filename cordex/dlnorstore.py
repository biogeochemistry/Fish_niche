...
models = {1: ('ICHEC-EC-EARTH', 'r3i1p1_DMI-HIRHAM5_v1'),2: ('ICHEC-EC-EARTH', 'r3i1p1_DMI-HIRHAM5_v1')}
...
for i in [1,2]:
    for j in [1]:
        for v in variables:
           s0,s1 = scenarios[i]
           m0,m1 = models[j]
           urlsA.append('%sLakes_%s%s%s_%s_%s_day_%s-%s.h5.gz' %  (url1, v, url2, m0, s0[0], m1, s0[1], s0[2]))
           urlsB.append('%sLakes_%s%s%s_%s_%s_day_%s-%s.h5.gz' % (url1, v, url2, m0, s1[0], m1, s1[1], s1[2]))
           print(urlsA)
urls = urlsA + urlsB
