function [ sediment_concentrations, sediment_params, sediment_matrix_templates, species_sediments] = sediments_init( pH, max_depth, temperature )
  % Input:
  ...
  % species_sediments - which species to simulate

    global sed_par_file sediment_params
    sediment_params = params(max_depth, temperature);
    sediment_concentrations = init_concentrations();
    sediment_matrix_templates = templates();
    species_sediments = species();
end

function [species_sediments] = species()
  ...
  species_sediments = containers.Map({species_sediments{:,2}},{species_sediments{:,1}});
end
...
function [sediment_concentrations ] = init_concentrations()
    global sediment_params
    n = sediment_params('n');

    % Init concentrations of sediments species
    O2      = ones(n,1) * 0.001;
    OM      = ones(n,1) * 0.001;
    OMb     = ones(n,1) * 0.001;
    NO3     = ones(n,1) * 10;
    FeOH3   = ones(n,1) * 0.001;
    SO4     = ones(n,1) * 10;
    NH4     = ones(n,1) * 0.1;
    Fe2     = ones(n,1) * 10;
    FeOOH   = ones(n,1) * 0.001;
    H2S     = ones(n,1) * 1E-10;
    HS      = ones(n,1) * 0.1;
    FeS     = ones(n,1) * 0.001;
    S0      = ones(n,1) * 0.001;
    ...
    S8      = ones(n,1) * 0.001;
    FeS2    = ones(n,1) * 0.001;
    AlOH3   = ones(n,1) * 0.001;
    PO4adsa = ones(n,1) * 0.001;
    PO4adsb = ones(n,1) * 0.001;
    Ca2     = ones(n,1) * 10;
    Ca3PO42 = ones(n,1) * 0.001;
    OMS     = ones(n,1) * 0.001;
    OH      = ones(n,1) * 0.1;
    CO2     = ones(n,1) * 100;
    CO3     = ones(n,1) * 100;
...
function [LU_solute, RK_solute, LD, LA, RD, RA] = matrices_template_solute(D_m, theta, v, fi, dx, dt, alpha, betta, n)
  ...
  LUnum = [-1 0 1];
  LU_solute = full(spdiags(LUdiags,LUnum,n-1,n-1));
  ...
  RKnum = [-1 0 1];
  RK_solute = full(spdiags(RKdiags,RKnum,n-1,n-1));
end
...
function [LU_solid, RK_solid, LD, LA, RD, RA] = matrices_template_solid(D_m, theta, v, fi, dx, dt, alpha, betta, n)
  ...
  LUnum_solid = [-1 0 1];
  LU_solid = full(spdiags(LUdiags_solid,LUnum_solid,n,n));
  LU_solid(1,2) = - 2*LD(1);
  ...
  RKnum_solid = [-1 0 1];
  RK_solid = full(spdiags(RKdiags_solid,RKnum_solid,n,n));
  RK_solid(1,2) = 2* RD(1);
