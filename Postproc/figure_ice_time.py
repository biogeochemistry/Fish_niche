...
lake_list = r"C:\Users\macot620\Documents\GitHub\Fish_niche\lakes\2017SwedenList.csv"
...
test.to_csv(r"C:\Users\macot620\Documents\GitHub\Fish_niche\lakes\icemodelvsdata1001120new_%s.csv"%timestr)
print("eee")

list_selected_lake = lake_id
test3 = data_lake
list_selected_lake = test3["ebhex"].tolist()
listmeanice = []
...
for eh in list_selected_lake:
    ...
    try:
        ice = pd.read_csv(os.path.join(outdir, 'His.csv'), names=['1', '2', '3', '4', '5', '6', '7', '8'])
        meanice11 = (ice['7'].sum()) / 10
        listmeanice1.append(meanice11)
        print(listmeanice1)
        # ice = pd.read_csv(os.path.join(outdir, 'His.csv'), names=['1', '2', '3', '4', '5', '6', '7', '8'])
        # meanice = (ice['7'].sum()) / 10
        # listmeanice.append(meanice)
        # print(test['ebhex'],test.loc[test['ebhex'] =='0x13fdf1'])
        # print( test3.loc[test3['ebhex'] == '0x13fdf1'])
        line = test.loc[test['ebhex'] == eh].reset_index()
        listmeanice.append((line['numday'][0]))
        # # print(len(line))
        # if len(line) != 0:
        #     # print(line['numday'][0])
        #
...
# test3['meanice'] = listmeanice
test3['numday'] = listmeanice

print(test)


timestr = time.strftime("%Y%m%d-%H%M%S")
test3.to_csv(r"C:\Users\macot620\Documents\GitHub\Fish_niche\lakes\icemodelvsdata100111old_%s.csv" % timestr)
print("eee")